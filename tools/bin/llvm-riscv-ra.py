    ...
    # get the code for each function
        ...
        for line in line_list:
            ...
            '...' in line:
                ...
                if fnname not in ProgFunction.fnlist or len(l) < 4:
                    continue
                op = l[2]
                p = l[3:]
...
class Inst:
    ...
    # We don't need to check for the "compressed" form of the instruction
    # (c.jal) because objdump prints the compressed instruction using its alias
    # (jal) by default.
        return inst == 'jal'

    def is_unconditional_branch(self, inst):
    # check if the given inst is an unconditional branch
    # We don't need to check for the "compressed" form of the instruction (c.j)
    # because objdump prints the compressed instruction using its alias (j) by
    # default.
        return inst == 'j'
    ...
    def inst_modifies_stack(self, op, p):
        sp = str(p)
        stack_ops = ['addi']
        return op in stack_ops and re.search('sp,sp,-[0-9]+', sp)
    ...
    def get_inst_cost(self, op, p, arch):
        inst_cost = 0
        sp = str(p)
        m = re.search('sp,sp,-([0-9]+)', sp)
        inst_cost = int(m.group(1))
        return inst_cost
...
parser = argparse.ArgumentParser(description='Snapdragon LLVM RISCV Resource \
Analyzer 1.0')
